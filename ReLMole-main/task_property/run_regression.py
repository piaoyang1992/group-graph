...
def parse_args():
    ...
    # training arguments
    parser.add_argument('--from_scratch', default=True, action='store_true', help="train from scratch")
    ...
    parser.add_argument('--epochs', default=100, type=int, help="number of training epoch")
...
def test(model, data_loader, device):
    ...
    with torch.no_grad():
        ...
        mae = mean_absolute_error(true_label, pred_score)
...
def main():
    ...
    for task in args.task_name_list:
        ...
        for k, (train_index, test_index) in enumerate(split):
            ...
            for epoch in range(1, args.epochs + 1):
                ...
                test_loss = test(model, test_loader, device)[eval_metric]
            ...
            stop_train_list = test(model, train_loader, device)
            # stop_val_list = eval(vocab_datas, raw_smiles_datas, model, device, valid_loader)
            stop_test_list = test(model, test_loader, device)

            test_result.append(stop_test_list[eval_metric])

            result_pd['train_' + str(k + 1)] = stop_train_list[eval_metric]
            # result_pd['val_' + str(k + 1)] = stop_val_list
            result_pd['test_' + str(k + 1)] = stop_test_list[eval_metric]
            print(result_pd)
    ...
    print(test_pd)
    test_pd.to_csv(result_path + '/' + 'regression_result.csv', index=False)
